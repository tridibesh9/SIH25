...
import { Leaf, TrendingUp, Award, BarChart3, RotateCw, Tag, Zap } from 'lucide-react';
import { ActiveCreditCard } from './ActiveCreditCard.jsx'; // Adjust path if needed
import { RetiredCreditCard } from './RetiredCreditCard.jsx'; // Adjust path if needed
...
export const Dashboard = ({ contract, account }) => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            ...
            try {
                console.log("Fetching owned and retired projects for account:", account);
                const [owned, retired] = await Promise.all([
                    ...
                    {activeProjects.length > 0 ? (
                        <div className="space-y-4">
                            {activeProjects.map(proj => (
                                <ActiveCreditCard key={Number(proj.projectId)} project={proj} contract={contract} ethToInrRate={ethToInrRate} />
                            ))}
                        </div>
                    ) : (
                    ...
                    {retiredProjects.length > 0 ? (
                        <div className="space-y-4">
                            {retiredProjects.map(proj => (
                                <RetiredCreditCard key={Number(proj.projectId)} project={proj} />
                            ))}
                        </div>
                    ) : (
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        ...
                        <h3 className="text-2xl font-bold text-gray-800 mb-1">{stats.totalCO2Offset.toLocaleString()}t</h3>
                        <p className="text-gray-600">Total CO₂ Retired</p>
                    </div>
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center mb-4"><BarChart3 className="w-6 h-6 text-purple-600" /></div>
                        <h3 className="text-2xl font-bold text-gray-800 mb-1">{activeProjects.length + retiredProjects.length}</h3>
                        <p className="text-gray-600">Total Projects Owned</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div className="border-b border-gray-200">
                        <nav className="-mb-px flex space-x-8 px-8">
                            {[ { id: 'portfolio', label: 'My Portfolio' }, { id: 'retired', label: 'Retired Credits' } ]
                            .map((tab) => (
                                <button
                                    key={tab.id}
